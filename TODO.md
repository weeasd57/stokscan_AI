...
- [ ] الحسابات الثقيلة تُحسب مسبقاً وتُحدّث بالكامل على Hugging Face

---

## 🔄 خطة: ضمان تغطية التشغيل اليومي لكل صفحات الموقع + تقارير التليجرام

**الهدف:** التأكد من أن `run_daily_bot_job.py` على Hugging Face يُحدِّث **كل بيانات كل صفحة** قبل ما المستخدمين يفتحوا الموقع الصبح، ويبعت تقرير مفصّل على التليجرام.

### 📋 خريطة الصفحات وتغطية التشغيل الحالية

| الصفحة | البيانات المطلوبة | يُحدَّث اليوم؟ | الخطوة المسؤولة |
|---|---|---|---|
| **الرئيسية `/`** | توصيات اليوم، ملخص السوق (EGX30)، أفضل الأسهم | ✅ جزئياً | Step 5 (توصيات) + Step 8 (market status) |
| **Scanner `/scanner`** | نتائج الـ AI scan، المؤشرات الفنية لكل الأسهم | ✅ جزئياً | Step 2 (indicators) + Step 5 (scan) |
| **Stocks `/stocks`** | أسعار وبيانات الأسهم الفردية، المؤشرات الفنية | ✅ | Step 1 (prices) + Step 2 (indicators) |
| **Chart `/chart`** | أسعار شموع تاريخية + مؤشرات رسومية | ✅ | Step 1 (prices) |
| **News `/news`** | أخبار السوق المحللة بـ AI (sentiment) | ✅ | Step 2.5 (news_sentiment) |
| **Admin `/admin`** | سجل التشغيل اليومي، إحصائيات الموديلات | ✅ | `daily_job_runs` table auto-saved |
| **Pro `/pro`** | محفظة المستخدم، الأداء التاريخي | ⚠️ جزئياً | Step 3 (positions) + Step 4 (evaluate) |
| **Blogs `/blogs`** | محتوى ثابت | ✅ لا يحتاج تحديث | — |
| **Profile `/profile`** | بيانات المستخدم | ✅ لا يحتاج تحديث | — |

### 📌 خطوات مفقودة أو تحتاج تطوير

#### 1. إشعار تليجرام بملخص التشغيل اليومي الكامل (Daily Digest)
- [ ] إضافة **STEP 10** في نهاية `run_daily_job()`: إرسال رسالة تليجرام واحدة شاملة تحتوي على:
  - ✅/❌ حالة كل خطوة (sync_prices، indicators، recommendations...)
  - عدد التوصيات الجديدة المولَّدة
  - حالة السوق (Bull/Bear) ونسبة تغيير EGX30
  - أي أخطاء أو تحذيرات حدثت
  - وقت التشغيل الإجمالي

#### 2. تحديث بيانات الـ Heatmap لصفحة Scanner
- [ ] إضافة **STEP 2.7**: حساب وتخزين بيانات الـ Sector Heatmap مسبقاً في Supabase (جدول `market_heatmap`) بعد انتهاء حساب المؤشرات مباشرة.
  - الآن هذه البيانات تُحسب عند الطلب عبر API وهذا يُبطّئ صفحة الـ Scanner.
...
#### 5. مراقبة وتنبيه أخطاء التشغيل (Error Monitoring)
- [ ] إضافة: لو فشل أي Step فشلاً كاملاً، يبعت تنبيه فوري منفصل على تليجرام (ليس في نهاية التقرير فقط).
- [ ] إضافة تحقق من أن `daily_job_runs` تُسجِّل كل تشغيل بحالة `completed` في آخر 24 ساعة، وإرسال تنبيه لو التشغيل تأخر أو فشل.

### 🗓️ أولوية التنفيذ

| الأولوية | المهمة | الأثر |
|---|---|---|
| 🔴 عاجل | STEP 10: Daily Digest Telegram Report | مراقبة يومية فورية |
