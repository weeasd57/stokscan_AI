...
const cases = [
    ...
    { email: "b.ahmed2113@gmail.com", messages: ["ابعتلى الاسهم اللى ينطبق عليها الشروط دى درجة التجميع أعلى من 75. نسبة الحجم أكبر من 1.5×. لا يوجد تصريف (Distribution = 0). التجميع مستمر يومين أو أكثر."] },
    { email: "abdallahsaied912@gmail.com", messages: ["Bioc مشترية عند 383", "مستهدفاته ايه"] },
    { email: "abdallahsaied912@gmail.com", messages: ["Caed ?"] }
];
...
for (const testCase of cases) {
    ...
    try {
        ...
        for (const message of testCase.messages) {
            ...
            report.push({ email: testCase.email, message, status: response.status, latency_ms: Date.now() - startedAt, reply: body.reply || body.detail || "", tables: body.tables || [], session_state: body.session_state || null });
        }
    } finally {
        if (sessionId && userId) {
...
fs.writeFileSync(new URL("./real_user_fix_verification.json", import.meta.url), JSON.stringify(report, null, 2));
