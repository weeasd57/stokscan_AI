...
class WhatsAppService:
        ...
        if self.enabled:
            try:
                ...
                if res.status_code in [200, 201]:
                    ...
                    logger.error(
                        f"WhatsApp API failed with status {res.status_code}: {res.text}"
                    )
    ...
    def _log_simulated_message(
        self, phone: str, message: str, error: Optional[str] = None
    ) -> bool:
        ...
        try:
            with open(WHATSAPP_LOG_PATH, "a", encoding="utf-8") as f:
                f.write(log_entry)
            print(
                f"[WhatsApp Simulated Gateway] Logged message to +{phone} in {WHATSAPP_LOG_PATH}"
            )
            return True
        except Exception as e:
