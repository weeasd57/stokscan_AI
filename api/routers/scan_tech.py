...
def get_sectors_heatmap(
    ...
    try:
        ...
        # Determine if we're fetching multiple dates or single date
        if start_date and end_date:
...
def _get_heatmap_date_range(symbol_pairs, company_names, start_date: str, end_date: str):
    """Get heatmap data for a date range - returns all frames"""
    from datetime import datetime, timedelta
    
    # Fetch historical technical indicators for the date range
    by_exchange = {}
    for symbol, exchange in symbol_pairs:
        by_exchange.setdefault(exchange, []).append(symbol)
            ...
            try:
                ...
                if res.data:
                    for row in res.data:
                        ...
                        tech_rows_by_date.setdefault(date, {})[key] = row
            except Exception as e:
                print(f"ERROR: heatmap range fetch failed for {exchange}: {e}")

    # Get all available dates and build frames
    all_dates = sorted(tech_rows_by_date.keys())
    frames_by_date = {}
    
    for date in all_dates:
        tech_rows = tech_rows_by_date[date]
        frame_data = _build_heatmap_frame(symbol_pairs, company_names, tech_rows, fundamentals, date)
        if frame_data['sectors']:  # Only include dates with data
            frames_by_date[date] = frame_data

    return {
        "frames_by_date": frames_by_date,
        "available_dates": list(frames_by_date.keys()),
        "start_date": start_date,
        "end_date": end_date,
        "total_frames": len(frames_by_date)
    }


def _get_heatmap_single_date(symbol_pairs, company_names, date: str):
    ...
    for exchange, symbols in by_exchange.items():
        ...
        for i in range(0, len(unique_symbols), 100):
            ...
            try:
                ...
                print(f"ERROR: heatmap single date fetch failed for {exchange}: {e}")
...
def _build_heatmap_frame(symbol_pairs, company_names, tech_rows, fundamentals, date):
    """Build a single heatmap frame from technical data"""
...
def _get_heatmap_latest(symbol_pairs, company_names):
    ...
    # Find latest date
    latest_date = None
    for tech in tech_rows.values():
        if tech and tech.get("date"):
            if not latest_date or tech.get("date") > latest_date:
                latest_date = tech.get("date")
    
    frame_data = _build_heatmap_frame(symbol_pairs, company_names, tech_rows, fundamentals, latest_date)
    return frame_data
