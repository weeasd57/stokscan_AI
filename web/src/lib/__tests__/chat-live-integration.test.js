...
describe("Live Supabase chatbot integration", () => {
    ...
    liveTest("handles advanced financial follow-ups without stale context", async () => {
        ...
        const turns = [
            ...
            ["الأسهم فوق القيمة الفنية", /فوق (?:القيمة الوسطية|منتصف النطاق|منتصف نطاقها)/],
            ["والأقل من القيمة العادلة", /(?:تحت|دون).*(?:القيمة الوسطية|منتصف)/s],
            ["هات الأسهم اللي تحت القيمة الفنية وفيها تصريف", /(?:تحت.*تصريف|تصريف.*تحت)/s],
            ["تحت القيمة مع تجميع", /(?:تحت.*تجميع|تجميع.*تحت)/s],
        ];
        try {
