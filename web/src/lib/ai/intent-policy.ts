...
export function isBestBuyStockQuestion(message: string): boolean {
    ...
    return /(?:افضل|أفضل|احسن|أحسن|ترشح|أشتري|اشتري|اشتريه|أشتريه|ادخل|أدخل|ادخله|أدخله|ايه\s+افضل|إيه\s+أفضل|ايه\s+احسن|مين\s+ادخل|مين\s+أدخل|مين\s+اشتري|مين\s+أشتري).{0,35}(?:سهم|أسهم|الأسهم|الاسهم|فرصة|فرصه|فيه|فيها|بكره|بكرة|النهاردة|النهارده|الجلسة|الجلسه|طالعة|طالعه|الاسبوع|الأسبوع|اسبوع|فرص)/i.test(value)
        || /(?:مين\s+(?:ادخله|أدخله|ادخل\s+فيه|أدخل\s+فيه|اشتريه|أشتريه)|ادخل\s+في\s+(?:مين|ايه|إيه)|اشتري\s+في\s+(?:مين|ايه|إيه))/i.test(value)
        || /^(?:ايه\s+افضل\s+سهم\s+للشراء|افضل\s+سهم\s+للشراء|أفضل\s+سهم\s+للشراء|اشتري\s+ايه\s+بكره|أشتري\s+إيه\s+بكرة|مين\s+ادخله\s+بكره|مين\s+أدخله\s+بكرة|نجم\s+الاسبوع|نجم\s+الأسبوع|القطاع\s+اللي\s+هيطلع|القطاع\s+اللي\s+يرتفع|السهم\s+اللي\s+هيرتفع|افضل\s+الفرص\s+المتاحة\s+حالياً|أفضل\s+الفرص\s+المتاحة\s+حالياً)/i.test(value);
}
...
export function isFairValueScanRequest(message: string): boolean {
    ...
    return /(?:الاسهم|اسهم|السهم|سهم).{0,45}(?:فوق|تحت|تحدت|تحدث|تكون|اقل|اعلي).{0,45}(?:القيمه|قيمه|قيمتها|التقييم).{0,20}(?:العادله|العادل|الفنيه|الوسطيه)/i.test(normalized)
        ...
        || /(?:تجميع).{0,35}(?:تحت|اقل|ارخص|رخيص).{0,35}(?:القيمه|قيمه|السعر|السعره|قيمته)/i.test(normalized)
        || /(?:تحت|اقل|ارخص).{0,35}(?:القيمه|قيمه|السعر|قيمته).{0,35}(?:تجميع)/i.test(normalized);
}
