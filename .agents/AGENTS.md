...
- **Rule [CRITICAL — DB Integrity]**: The `stock_technical_indicators` table in Supabase MUST always contain exactly ONE row per stock symbol. It stores only the LATEST trading day snapshot — it is NOT a historical time-series table. Any daily job writing to this table MUST delete the existing row for each symbol before inserting (or use upsert on `symbol` as the primary conflict key). Never allow multiple rows for the same symbol. If duplicates are found, they must be deduplicated immediately by keeping only the row with the latest `date` value per symbol.
